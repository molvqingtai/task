...
describe('Test task', () => {
  ...
  describe('Test sync task', () => {
    ...
    test('should call callback times and restart', async () => {
      ...
      expect(callback1).toHaveBeenCalledTimes(1)
      expect(callback2).toHaveBeenCalledTimes(1) // 0
      expect(callback3).toHaveBeenCalledTimes(0)
      expect(callback4).toHaveBeenCalledTimes(0)
      ...
      expect(callback1).toHaveBeenCalledTimes(1)
      expect(callback2).toHaveBeenCalledTimes(1)
      expect(callback3).toHaveBeenCalledTimes(1) // 0
      expect(callback4).toHaveBeenCalledTimes(0)
      task.start()
  ...
  describe('Test async task', () => {
    ...
    test('should return callback status', async () => {
      ...
      expect(task.status).toBe('stopped')
      expect(task.query('1')?.status).toBe('pending')
      expect(task.query('2')?.status).toBe('pending')
      expect(task.query('3')?.status).toBe('pending')
      task.start()
      expect(task.status).toBe('running')
      await vi.advanceTimersByTimeAsync(1500)
      expect(task.query('1')?.status).toBe('fulfilled')
      expect(task.query('2')?.status).toBe('pending')
      expect(task.query('3')?.status).toBe('pending')
      task.pause()
      expect(task.status).toBe('paused')
      ...
      await vi.advanceTimersByTimeAsync(2000)
      expect(task.query('1')?.status).toBe('fulfilled')
      expect(task.query('2')?.status).toBe('fulfilled')
      expect(task.query('3')?.status).toBe('running')
      await vi.advanceTimersByTimeAsync(2000)
      task.stop()
      expect(task.status).toBe('stopped')
      expect(task.query('1')?.status).toBe('fulfilled')
      expect(task.query('2')?.status).toBe('fulfilled')
      expect(task.query('3')?.status).toBe('rejected')
    })
  })
})
